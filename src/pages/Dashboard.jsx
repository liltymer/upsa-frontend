...
export default function Dashboard() {
    ...
    const fetchAll = async () => {
      try {
        const [dashData, riskData, trendData] = await Promise.all([
          getDashboard(), getRisk(), getTrends(),
        ]);
  ...
  const getTrendStyle = (trend) => {
    if (trend === "Improving") return { color: "var(--green)", icon: "📈", bg: "var(--green-bg)" };
    if (trend === "Declining") return { color: "var(--red)", icon: "📉", bg: "var(--red-bg)" };
    return { color: "var(--amber)", icon: "➡️", bg: "var(--amber-bg)" };
  };
  ...
  const features = [
    { label: "My Results", sub: "Add, edit and manage your semester results", path: "/results", icon: "📋", color: "var(--blue-bg)", border: "var(--blue-border)" },
    { label: "GPA Tracker", sub: "View your GPA history and semester breakdown", path: "/gpa", icon: "📊", color: "var(--green-bg)", border: "var(--green-border)" },
    { label: "Transcript", sub: "View full academic record and download PDF", path: "/transcript", icon: "📄", color: "var(--orange-bg)", border: "var(--orange-border)" },
    { label: "GPA Simulator", sub: "Simulate future CGPA and calculate target grades", path: "/simulator", icon: "🎯", color: "#FDF4FF", border: "#E9D5FF" },
    { label: "Risk Analysis", sub: "Check your academic risk level and alerts", path: "/risk", icon: "⚡", color: "var(--amber-bg)", border: "var(--amber-border)" },
  ];
