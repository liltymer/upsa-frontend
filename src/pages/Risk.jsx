...
export default function Risk() {
  ...
  useEffect(() => {
    const fetchRisk = async () => {
      try {
        ...
        setError("Failed to load risk analysis. Please refresh.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const getRiskStyle = (level) => {
    if (level === "High") return {
      ...
      border: "var(--red-border)", dot: "#EF4444",
      icon: "🔴", label: "High Risk",
      desc: "Immediate action required.",
    };
    if (level === "Medium") return {
      color: "var(--amber)", bg: "var(--amber-bg)",
      border: "var(--amber-border)", dot: "#F59E0B",
      icon: "🟡", label: "Medium Risk",
      desc: "Your performance needs attention.",
    };
    return {
      color: "var(--green)", bg: "var(--green-bg)",
      border: "var(--green-border)", dot: "#22C55E",
      icon: "🟢", label: "Low Risk",
      desc: "You are in good academic standing.",
    };
  };

  const getClassStyle = (cgpa) => {
    if (cgpa >= 3.6) return { label: "First Class", color: "var(--green)", bg: "var(--green-bg)", border: "var(--green-border)", range: "3.6 – 4.0" };
    if (cgpa >= 3.0) return { label: "Second Class Upper", color: "var(--blue)", bg: "var(--blue-bg)", border: "var(--blue-border)", range: "3.0 – 3.59" };
    if (cgpa >= 2.5) return { label: "Second Class Lower", color: "var(--amber)", bg: "var(--amber-bg)", border: "var(--amber-border)", range: "2.5 – 2.99" };
    if (cgpa >= 2.0) return { label: "Third Class", color: "var(--orange)", bg: "var(--orange-bg)", border: "var(--orange-border)", range: "2.0 – 2.49" };
    if (cgpa >= 1.0) return { label: "Pass", color: "var(--text-muted)", bg: "#F9FAFB", border: "var(--border)", range: "1.0 – 1.99" };
    return { label: "Fail", color: "var(--red)", bg: "var(--red-bg)", border: "var(--red-border)", range: "0.0 – 0.99" };
  };
  ...
  const currentBandIdx = classBands.findIndex((b) => cgpa >= b.min);
    ...
    <div style={{
      ...
      <div className="page-header">
        <div className="header-inner" style={{
          ...
          >
            <p style={{ fontSize: 32, marginBottom: 6 }}>{riskStyle.icon}</p>
            <p style={{
              fontFamily: "var(--font-heading)",
              fontWeight: 800, fontSize: 18,
              color: riskStyle.color, marginBottom: 4,
            }}>
              {riskStyle.label}
            </p>
            <p style={{
      ...
      <div className="overlap-section">
        <div className="grid-3">
          ...
          {/* Current CGPA */}
          <div className="card-dark stat-card fade-up-1"
          ...
          <div className="stat-card card fade-up-2">
            ...
            <div>
              ...
              <p className="stat-value" style={{
                fontSize: 32,
                color: gap != null && gap <= 0 ? "var(--green)" : "var(--navy)",
              }}>
                {gap != null ? `+${gap}` : "—"}
              </p>
              <p className="stat-sub" style={{ color: "var(--text-muted)" }}>
                {gap != null && gap <= 0
                  ? "Already at top class! 🎉"
                  : `To reach ${nextClass || "next class"}`}
              </p>
            </div>
          </div>
      ...
      <div className="page-content" style={{ marginTop: 28 }}>
        <div className="grid-main">
          ...
          {/* RIGHT COLUMN — Classification Ladder */}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 12,
                      padding: "14px 16px",
                      background: isYou ? band.bg : "var(--bg-page)",
                      border: `1.5px solid ${isYou ? band.border : "var(--border)"}`,
                      borderRadius: "var(--radius-md)",
                      transition: "var(--transition)",
                      position: "relative",
                    }}
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{
                        ...
                        fontSize: 13,
                        color: isYou ? band.color : "var(--text-secondary)",
                        marginBottom: 2,
                      }}>
                        {band.label}
                      </p>
                      <p style={{
                        ...
                        color: isYou ? band.color : "var(--text-muted)",
                        opacity: isYou ? 1 : 0.7,
                      }}>
                        {band.range}
                      </p>
